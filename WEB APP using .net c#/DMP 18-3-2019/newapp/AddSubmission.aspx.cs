...
public partial class newapp_AddSubmission : System.Web.UI.Page
{
    ...
    public void SaveFilePath(string name, int size, string path)
    {
        ...
        string insertText = "INSERT INTO TYCOAssignmentsStud(id,FileName,FileSize,FilePath) VALUES ((select isnull(max(id),0)+1 from TYCOAssignmentsStud),@FileName,@FileSize,@FilePath)";
            ...
            if (correctExtension)
            {
                if (fileExtension == ".pdf" || fileExtension == ".doc" || fileExtension == ".docx" || fileExtension == ".txt" || fileExtension == ".xlsx" || fileExtension == ".jpg" || fileExtension == ".png")
                {
                    try
                    {
                        ...
                        Response.Redirect("FinalAddingStud.aspx");
