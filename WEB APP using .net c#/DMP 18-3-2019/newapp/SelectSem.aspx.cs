...
public partial class newapp_SelectSem : System.Web.UI.Page
{
    ...
    protected void Upload_ServerClick(object sender, EventArgs e)
    {
        ...
        SqlCommand cmd = new SqlCommand("insert into forjoin(id,Sem,Sub,UploadDate,SubmissionDate,Asn) values ((select isnull(max(id),0)+1 from forjoin),@Sem,@Sub,@UploadDate,@SubmissionDate,@Asn)", con);
        ...
        cmd.Parameters.AddWithValue("@SubmissionDate", Calendar1.SelectedDate);
        cmd.Parameters.AddWithValue("@Asn", TextBox3.Text);
